        ...
        for f in files:
            ...
            if file_text in ('.mp4', '.m4a', '.jpg', '.txt', '.srt', '.opus', '.webm', '.mkv'):
                ...
                newname = newname.replace('寶寶巴士', '').replace('宝宝巴士', '').replace('BabyBus', '').replace(
                    '_卡通片', '').replace(
                    '+更多', '_').replace('_合集', '').replace('_片', '').replace('片片', '').replace(' ', '').replace(
                    '_.', '.').replace('__', '_').replace('__', '_').replace('＋更多合集', '')
        ...
        for f in files:
            ...
            if file_text in ('.mp4', '.m4a', '.jpg', '.txt', '.srt', '.opus', '.webm', '.mkv'):
                ...
                if not os.path.exists(dstpath):
                    ...
                    if not os.path.exists("/volume1/WD8T/巴士回收/" + f):
                        print('文件已存在,移动回收站:' + dstpath)
                        os.rename(srcpath, "/volume1/WD8T/巴士回收/" + f)
                    else:
...
if __name__ == '__main__':
    ...
    # 目的路径
    dst_path = r'/volume1/WD8T/Media/儿童/宝宝巴士Youtube'
    # 下载路径
    path = r'/volume1/WD8T/Media/temp'
    # yt = YouTube('https://www.youtube.com/watch?v=cowy1pou3QY')
    # 从网站获取更新
    logs = []
    onlinelist = getid(ytweb)
    # 读取本地已下载记录
    loclist = open('id_download_log.csv', 'r', encoding='UTF-8').read().replace('[', '').replace(']', '').replace("'",
                                                                                                                  '').replace(
        ',', '').split()
    # 开始下载并更新本地记录
    for id in onlinelist:
        ...
        if id not in loclist:
            ...
            loclist.append(id)
            if d == 0:
                idf = open('id_download_log.csv', 'w', encoding='UTF-8')
                idf.write(str(loclist))
                print('本地数据已更新')
                logger('已下载')
